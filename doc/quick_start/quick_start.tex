...
% \item Press the tab \texttt{Geometry} in the right side menu and click on the 
% \teclapuntos key. A window will be opened with the available \ggbs{ } and 
% {\it .xyz} files of
% the project containing the geometry (see fig \ref{fig:22}). 
% {\bf Do not choose {\it -cps-d.xyz} or 
% {\it -cps-v.xyz} files}, since they correspond to critical points coordinates,
% not to atoms coordinates. Open the file by double clicking on the name or
% clicking once and pressing the \openkey key. A figure with the molecule will be
% visualized. You can choose among four schemes for visualizing the molecular 
% skeleton. Fine tuning of the display is also possible.
% Use the mouse buttons for reorienting or displacing the molecule
% (this can be done also in the \texttt{Options} tab; see fig \ref{fig:23}).
% 
% \item Press the tab \texttt{Geometry measurements} in the right side menu
% and check the \texttt{Distances} radio button. Check also the boxes labeled as
% {\it Show/hide distances in a window} and {\it Show/hide distances in the viewer}.
% Next, hold the shift key and double click on a pair of centers.
% Distance between the selected centers will be displayed both in the viewer and in
% a pop window (see fig \ref{fig:24}). You can repeat with angles (choosing three 
% centers) and dihedral angles (four centers). Angles can be displayed only in a 
% pop window(see fig \ref{fig:25}).
% 
% \item Press the tab \texttt{Surfaces} in the right side menu and click on the 
% \teclapuntos key for selecting a 3D grid to be visualized (files \plt{ }). A
% window with the \plt{ } files available will be opened (see fig \ref{fig:26}).
% Choose one by double clicking on it. Use the ruler or the box to choose a
% contour value. To change sign of contour, press on the \pluskey or
% \minuskey keys. The contour surface will be dynamically rendered (see fig
% \ref{fig:27}). Each contour to be displayed must be
% selected in a different
% tab. Up to 8 contours can be visualized at the same time. Single contours can
% be shown or hidden by checking (unchecking) the \texttt{Show/hide contour surface} checkbox.
% 
% \item Hide surfaces by unchecking the boxes labeled \texttt{Show/hide contour surface}
% (see fig \ref{fig:28}).
% 
% \item Press the tab \texttt{Molecular topography} in the right side menu(see fig \ref{fig:29}). 
% Click on the \teclapuntos keys for selecting a {\it -cps-d.xyz} or 
% {\it -cps-v.xyz} file with the critical points (CP) of density (the former) or electrostatic
% potential (the latter) to be visualized. (See fig \ref{fig:30}). CP symbols, indices
% and field value at CP can be also displayed for all CPs (see fig \ref{fig:31}), or only for
% selected ones  (see fig \ref{fig:32}).
% Hessian eigenvectors at CPs can be displayed as well (see fig \ref{fig:33}).
% Choose a file {\it -d.gpdat} or {\it -v.gpdat} with a gradient path to display gradient 
% lines of density or electrostatic potential (see fig \ref{fig:34}).
% Choose a file {\it -d.bswir}, {\it -d.bssol}, {\it -v.bswir} or {\it -v.bssol}
% in the \texttt{Atomic basins} section to display atomic basins (see fig \ref{fig:35}).
% Hide CPs, atomic basins, gradient lines and hessian eigenvectors by unchecking
% the pertaining boxes.
% 
% \item Press the tab \texttt{Forces} in the right side menu and 
% click on the \teclapuntos keys for selecting a {\it .forces} file with the 
% Hellmann-Feynman forces to be visualized. Increase vectors lengths if required
% (See fig \ref{fig:36}). Hide vectors by unchecking the boxes.
% 
% \item Press the tab \texttt{Field lines} in the right side menu and click on
% the \teclapuntos keys for selecting the file with the electric field lines to be
% visualized. (See fig \ref{fig:37}).
