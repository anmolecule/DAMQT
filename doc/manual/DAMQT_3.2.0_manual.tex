...
Sources can be modified and distributed according to GPL terms.
...
\xml{ } file, a NWCHEM\index{NWCHEM} output file, {\it .nwcout}, or a MOPAC\index{MOPAC}
{\it .aux} file
...
\item Holding the \texttt{Shift} key pressed and moving the mouse, a rectangular
...
The menu contains eleven items: {\it Add molecule}, {\it Geometry measures}, {\it Rotations}, {\it 
...
with
gOpenMol and other packages. The line after NCEN must be empty or contain a comment and it will not processed.
...
(laboratory frame). It is important to distinguish this behavior to that corresponding to the transformation of
...
the laboratory frame, or those in the molecular editor --see section {sec:4.13}-- for the local molecular frames.
...
\begin{figure}[H]
\begin{center}
\includegraphics[width=0.20\linewidth]{damqt320_measures.png}
\end{center}
...
In this case, when more than one molecule has been loaded in the viewer, the tetrads of 
centers are collected within parenthesis
with a subscript indicating the molecule order index.
...
\begin{figure}[H]
    \begin{center}
        % \vspace*{-5mm}
        \includegraphics[width=0.5\linewidth]{damqt320_capture.png}
    \end{center}
%     \vspace*{-1mm}
...
\begin{minipage}{.45\linewidth}
    \begin{figure}[H]
        \begin{center}
            \vspace*{0mm}
            \includegraphics[width=0.5\linewidth]{damqt320_lights.png}
        \end{center}
        \vspace*{1mm}
...
\begin{minipage}{.3\linewidth}
    \begin{figure}[H]
        \begin{center}
%             \hspace*{-5mm}
            \includegraphics[width=0.79\linewidth]{damqt320_balls_sticks.png}
        \end{center}
        \vspace*{5mm}
...
\begin{minipage}{.3\linewidth}
    \begin{figure}[H]
        \begin{center}
            \vspace*{0mm}
            \includegraphics[width=0.7\linewidth]{damqt320_viewport.png}
        \end{center}
        \vspace*{5mm}
...
\begin{figure}[H]
    \begin{center}
        % \vspace*{-5mm}
        \includegraphics[width=0.74\linewidth]{damqt320_3D_settings.png}
    \end{center}
    \vspace*{5mm}
...
cluster building using the EPIC method as proposed by Gadre et al. 
...
added to the canvas. If only one molecule is present, it will act as host.
...
\begin{figure}[H]
    ...
    \end{center}
    \vspace*{5mm}
    \caption{Save/retrieve \label{fig:4_10_3}}
\end{figure}
\end{minipage}
\end{tabular}

There are two ways to build a cluster. The first one can be chosen by 
checking the button under the legend {\it  Choose guests from canvas}, and
...
the movements. Two interpolation schemes are implemented: one based on interpolating 
quaternions used for rotations, and another impying a linear 
interpolation of coordinates.
The first scheme, albeit a bit more lengthy, is preferred as it keeps the
geometries of the molecules undistorted (only rotations and translations
of the molecule as a whole are allowed). On the contrary, linear interpolation
may cause distortions on the molecular geometries and 
the appearance of {\it ghost} bonding in the interpolated frames.
...
coordinates of all molecules displayed in the canvas.
...
shown in fig \ref{fig:4_13_2}. Eight options for edition appear in the menu: 
{\it Molecular skeleton}, {\it Labels}, {\it Rotations},
{\it Translations}, {\it Field lines}, {\it Critical points}, {\it Add surface}, and
...
\begin{minipage}{.35\linewidth}
    \begin{figure}[H]
        \begin{center}
            % \vspace*{-5mm}
            \includegraphics[width=0.5\linewidth]{damqt320_rotations}
        \end{center}
%         \vspace*{16mm}
...
rotation around the space-fixed $y$ axis, and vertical displacements, rotation
around the space-fixed $x$ axis.
\item Holding the {\it right button}, mouse displacements cause
rotation around the space-fixed $z$ axis.
...
\item Holding the \texttt{Ctrl} key and double clicking on a molecule structure or surface toggles molecule activation\index{3D graphics!molecule activation}.
...
UHF\index{MOLPRO interface!UHF orbitals} calculations are different in \out{ } file 
from those of \xml{ } file. In \out{ } file appear separated in two sets corresponding 
...
just mentioned without much detail, and we will focus here in what are more 
specific tasks for cluster optimization usinf EPIC.
...
This distinction is important, as more information required for host than for 
guests.

\item Start a project from a molecular calculation for the host with any 
standard package compatible by DAMQT.
...
(either active or inactive) are placed in the same region, the latter
only acts on the molecule loaded first.
...
\item Keeping the option {\it Choose guest from canvas} checked, press
...
at the starting, intermediate and ending points. The path and name 
of this file are supplied
in suitable boxes --see fig \ref{fig:A6_5}. 

\item File {\it *.xyz\_frames} can be used
for animation by loading in the pertaining box --see fig \ref{fig:A6_5}.
...
Meanwhile recording is in progress, a message will appear as
...
optimizing to change the viewpoint --see fig \ref{fig:A6_9}.
