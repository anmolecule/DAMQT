!  Copyright 2013-2019, Jaime Fernández Rico, Rafael López, Ignacio Ema,
...
Latest version is available upon request to the authors at the following
address:
...
The package is delivered as a tarball with the source files and a sample deck of input and output files. 

The parallel versions have been prepared using MPI and tested with mpich2  1.4.1p1

Installation
------------

A fortran90 compiler is required for the scalar version, and an fortran90 MPI compiler is 
necessary for the parallel version. The current version has been tested on Intel's ifort
and GNU's gfortran, and on mpich2 1.4.1p1.

A C++ compiler and Qt library (5.9 or higher) is required for the interfaces.

CMakeLists.txt files are included to generate and install the executables using cmake. 
Use preferably cmake-gui utility to check that the variables defining the compilers 
and libraries are pointing to appropriate addresses.

To install executables (default in /usr/local/bin) type "make install".
This operation may require root privileges.

To uninstall the package, type "make uninstall".

To remove executables, type "make clean".
...
Otherwise, these data will be supplied in the standard input, after the projectname, according to the
following structure:
...
RECORD : nbasis (number of basis functions, integer*4)
...
Options in namelist and defaults:
    ...
    linpgen = .false.        if .false. reads geometry and basis set from  file  'projectname'.sgbs  
                            and density matrix from  file 'projectname'.den as generated by program  m2c (SMILES)
                            otherwise, reads it from standard input as described above
...
DAM320 also generates an unformatted file "projectname.damqt" with the data of the density fit to be used by the remaining 
programs of the package. The content of this file can be read with the ancillary program "readdamqt320" also included in 
...
G-DAM320 also generates a unformatted file  projectname.damqt  with the data of the density fit to be used by the remaining 
programs of the package. The content of this file can be read with the ancillary program leedamqt also included in 
the package (leedamqt.f90). This file requires as input data the projectname and writes the content of 
the file projectname.damqt as text to the standard output.
...
If the grid option is selected, generates files with extension .plt containing the values of the several 
...
Optionally (if lauto .ne. .true., see options below) it can read the data of the field lines to be plotted from file
projectname.lin whose records contain the lines especification as follows:

RECORD I: icnt, x0, y0, z0, dlt0     

where icnt is a center index (integer*4), x0, y0, z0 are Cartesian coordinates of a point in the line (real*8) and dlt0 the 
step size (real*8)

The dlt0 value is required only if the individual steps option has been selected (lstepindiv = .true., see options below)

If icnt .eq. 0, x0, y0, y0 is the starting point for the line. Otherwise, the line starts at the center icnt and its second 
point is x0, y0, y0. 
...
Options in namelist and defaults:
    ...
    numlns = 100            Maximum number of lines
    
    lautolines = .true.        If .true. automatically generates the lines. If .false. reads lines from a file *.lin
    
    largo = .false.        If .true. computes the long-range field
    
    larrows = .true.        If .true. plots arrows in lines separated narrstep   steps
    
    narrstep = 100            Number of steps for plotting arrows
    
    rlongarr = .25d0        Arrows length
    
    rwidearr = .125d0        Arrows width
    
    lstepindiv = .false.      If .true. step length is provided for each individual line (dlt0) as well as the arrows 
                            separation (narrstep)
...
A file projectname.cam is generated containing the data of electric field lines. The lines are given as sets of coordinates
x, y, z of the points in the line. Different lines are separated by blank lines. Arrows are defined in the same way.
...
It also generates a file:

projectname.forces: 

which contains the components of the Hellmann-Feynman forces over nuclei in the following order: 
...
Options in namelist and defaults:
    ...
    lderiv = .false.        f .true. tabulates the first derivative of the radial factors
...
Options in namelist and defaults:
    ...
    lmin = mxlexp            highest order of multipolar moments to be rotated
    
    ncntab                 number of centers for tabulations (apart from the centers used to define the frame)
    
    icntab                 indices of centers to be tabulated
