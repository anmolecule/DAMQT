...
!    End of namelist defaults
    ...
    if (myrank .eq. 0) then
        ...
        if (lexact) then
            write(6,"(//'This version of the program is not prepared for exact potential. Run DAMPOT320_mpi instead.',//)")
            abort = 1
        endif
    endif
...
!
  subroutine error(ierr, msg)
    USE MPI
    USE DAM320_D
    USE DAM320_DATA_D, ONLY: projectname
    USE DAMPOT320_D, ONLY: filename
    implicit none
