...
!    	open (iout, file = trim(filename)//"-DAMTOPO320_mpi.out", status="replace") ! Sealed by Rafa
        inamelist = (/ lmaxi /)
...
!	These serve as guess points to MESP.   
    ...
    CALL MPI_BARRIER(MPI_COMM_WORLD,ierr)
    if (myrank.eq.0) then
        call system("cat "//trim(filename)//".ts_?? > "//trim(filename)//".ts")
        call system("rm -f "//trim(filename)//".ts_??")
...
! ----------      change made by Rafa          -----------------------------            
            ...
            CALL SYSTEM("cat "//trim(filename)//".gcp_"//"?? > "//trim(filename)//".gcp")
            CALL SYSTEM("rm -f "//trim(filename)//".gcp_"//"??")
! ----------      End of change made by Rafa   ----------------------------  
        end if
    end do
...
! 	CPs which are 0.1 au away from each other are considered to be different  
    if (myrank .eq. 0) then
! ----------      change made by Rafa          -----------------------------      
        CALL SYSTEM("cat "//trim(filename)//".tcp_"//"?? > "//trim(filename)//".tcp")
        CALL SYSTEM("rm -f "//trim(filename)//".tcp_"//"?? "//trim(filename)//".gcp")
...
! 	on top of their respective category
    if (myrank.eq.0) then
        if (med) call system("rm -f "//TRIM(filename)//"-cps-d.xyz "//TRIM(filename)//".tfcp")
        if (mesp) call system("rm -f "//TRIM(filename)//"-cps-v.xyz "//TRIM(filename)//".tfcp")
        if (med) call system("rm -f "//TRIM(filename)//"-cps-d.eigv")
        if (mesp) call system("rm -f "//TRIM(filename)//"-cps-v.eigv")
        call system("cat "//TRIM(filename)//".tfcp_?? > "//TRIM(filename)//".tfcp")
        call system("rm -f "//TRIM(filename)//".tfcp_??")
...
! 		Broadcasting the character array of CP symbols as integers.     
    ...
    open ( irmp, file=trim(filename)//trim(adjustl(irankd))//".rmdat",status="replace")
...
8	format(3(f6.2,1x))                  
    ...
    if (myrank.eq.0) then
        ...
        CALL SYSTEM("cat "//trim(filename)//"??.rmdat > "//trim(filename)//".rmdat")
        CALL SYSTEM("rm -f "//trim(filename)//"??.rmdat")
...
!        Modified by Rafa 2019  -------------------------------------------------------------------------------------------
      ...
      open (unit=IBILQT+1, file=trim(filename)//trim(adjustl(irankd))//".vert",status="replace", iostat=ierrqt)
      if (ierrqt .ne. 0) then
          write(6,*) 'Error when opening file ', trim(filename)//trim(adjustl(irankd))//".vert"
          abort = 1
          return
      endif
!        End of modification by Rafa 2019  ----------------------------------------------------------------------------------

    IF (SOLIDSURF) OPEN(IBILS,file=trim(filename)//trim(adjustl(irankd))//".ssurf",status="replace")
    IF (WIREFRAME) OPEN(IBILW,file=trim(filename)//trim(adjustl(irankd))//".wsurf",status="replace")
...
!        Modified by Rafa 2019  -------------------------------------------------------------------------------------------
    if (myrank .eq. 0) then
        call system("cat "//trim(filename)//"??.vert > "//trim(filename)//".vert")
...
CALL MPI_BARRIER(MPI_COMM_WORLD,ierr)
    ...
    if (myrank.eq.0) then
        if (med.and.solidsurf) CALL SYSTEM("cat "//trim(filename)//"??.ssurf > "//trim(filename)//"-d.bssol")
        if (mesp.and.solidsurf) CALL SYSTEM("cat "//trim(filename)//"??.ssurf > "//trim(filename)//"-v.bssol")
        if (med.and.wireframe) CALL SYSTEM("cat "//trim(filename)//"??.wsurf > "//trim(filename)//"-d.bswir")
        if (mesp.and.wireframe) CALL SYSTEM("cat "//trim(filename)//"??.wsurf > "//trim(filename)//"-v.bswir")
        CALL SYSTEM("rm -f "//trim(filename)//"??.ssurf")
        CALL SYSTEM("rm -f "//trim(filename)//"??.wsurf")
    end if
    RETURN
    END SUBROUTINE
!---------------------------------------------------------------------------------
